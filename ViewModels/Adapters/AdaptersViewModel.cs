...
namespace AIStudio.ViewModels.Adapters
{
  ...
  public sealed class AdaptersViewModel
  {
    ...
    private string _reportText = "Выберите пакет и нажмите «Проверить», либо создайте новый из demo.";
    ...
    public string AdaptersCatalogHint =>
        "Зарегистрированные пакеты: " + AdapterPaths.AdaptersRootPath
        + ". Новый пакет: «Создать из demo…» (каркас " + AdapterPaths.GetDemoTemplatePath()
        + "). Двойной щелчок по строке — свойства; Delete — удалить пакет.";
    ...
    /// <summary>Открывает редактор создания пакета из demo.</summary>
    public void CreateFromDemo(Window owner)
    {
      ...
      if (!AdapterPackageManager.TryCreateFromDemoTemplate(
              dialog.EditedManifest,
              out string installedPath,
              out string error))
      {
        MessageBox.Show(error, "Создать из demo", MessageBoxButton.OK, MessageBoxImage.Warning);
        return;
      }

      Reload();
      SelectById(dialog.EditedManifest.Id);
      ReportText = "Создан пакет из demo:\n" + installedPath
          + "\n\nЗаполните runtime\\ и при необходимости schema, затем «Проверить».";
      MessageBox.Show(
          "Пакет «" + dialog.EditedManifest.DisplayName + "» создан:\n" + installedPath,
          "Создать из demo",
          MessageBoxButton.OK,
    ...
    private static AdapterManifest CreateDefaultManifest()
    {
      return new AdapterManifest
      {
        ...
        SchemaVersion = "1.0",
        Description = "Каркас пакета. Заполните schema и runtime, затем «Проверить»."
      };
    }
