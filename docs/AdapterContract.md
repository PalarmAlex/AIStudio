...
| [`AdapterAuthorGuide.md`](AdapterAuthorGuide.md) | Практика сборки пакета |
...
- combobox и подсказки из `schema\` — **ProbeKey** в `InfluenceActions.dat` (редактор «Метрики среды (EA)»), каталог рецептов из `recipe-catalog.json`.
...
| **Mechanical (среда)** | Метрики host, SessionHealth | `InfluenceActions.dat` (ProbeKey, ID ≥ 50) → фаза A/B на пульсе Velum | **Нет** (фаза B через EA skip influences) |
| **Наблюдаемая команда SW** | Command buffer host | `CommandPatternIdList` | **Да** (отдельно от mechanical delta) |

- Событие SW → гомеостаз — **фаза A** (composer по ProbeKey) и **фаза B** (триггер рефлекса через EA с ProbeKey, influences skip в ISIDA); **не** auto EA оператора.
...
| **Runtime host** | Command idle-flush; metric probe orchestrator (A/B); G_AD → рецепт на `OnPulseCompleted` |
...
{projectRoot}\
  ...
  Data\                       — ISIDA (.dat), incl. AdaptiveActions.dat, CommandPhrases.dat, InfluenceActions.dat (ProbeKey)
```
...
Пуск по Command SW — `GeneticReflexes.dat` → `command_pattern_ids`; mechanical path — `InfluenceActions.dat` (строки с **ProbeKey** из `metric-probes.json`).
...
Слой **`EnvironmentTriggers.yaml`** удалён. Mechanical path — **`InfluenceActions.dat`** (ProbeKey, EA ≥ 50). Файл **`EnvironmentPressureRules.dat`** снят (legacy). Пуск Command — **`GeneticReflexes.dat`** → `command_pattern_ids`. Файлы `schema/trigger-detect.json` и `schema/trigger-catalog.json` **запрещены** в пакете.
...
| `metric-probes.json` | ProbeKey в `InfluenceActions.dat` (метрики среды) |
...
Студия заполняет combobox **ProbeKey** в `InfluenceActions.dat` из `probes[].key` (редактор **«Метрики среды (EA)»**; плюс «оператор, не среда» в полном справочнике стимулов).

**Operator path:** дискретные стимулы оператора — каналы Verbal/Command/Visual + EA без ProbeKey. **Mechanical path:** строки EA с ProbeKey (ID ≥ 50) — шаблон давления на виталы (фаза A) и триггер рефлекса (фаза B); runtime Velum orchestrator.
...
### 10.8. Панель задач Velum (вкладка «Агент»)

Runtime host (SolidWorks Task Pane, `VelumAgentTaskPane`) — операторский пульт агента в CAD:

| Элемент | Поведение |
|---------|-----------|
| **Шапка** | Состояние гомеостаза (ПЛОХО/НОРМА/ХОРОШО), индикатор CAD, команды Velum (Старт/Стоп/Настройки), кнопки Н/В |
| **Сворачиваемый блок** | «Состояние и метрики среды агента» — шапка + мозаика плашек метрик из `InfluenceActions.dat` (строки с ProbeKey) |
| **Плашки метрик** | **Красный** — активная метрика давит (probe плохая); **голубой** — активна, не давит; **серый** — отключена (`Active=false`) |
| **Tooltip** | ID, имя, описание (отдельные строки) |
| **Двойной клик** | Справка: ID, имя, описание, активность, давление на параметры (имя/ID и сила) |
| **Контекстное меню** | Включить / выключить метрику; цвет обновляется сразу; отключённые не опрашиваются и не давят |
| **Ниже** | Буфер Command SW, воздействия оператора, вербальный ввод, ответ агента |

Поле **`Active`** в `InfluenceActions.dat` (7-й столбец, `true`/`false`) редактируется в AIStudio: «Среда» → «Метрики среды».
...
| Mechanical path (метрики → P_i) | ISIDA / AIStudio / Velum | `InfluenceActions.dat` (ProbeKey) |
...
- [ ] Mechanical path → `InfluenceActions.dat` (ProbeKey) → P_i
...
*Контракт 3.2 согласован с [`SymbiontArchitecture_IsidaAdapter.md`](../../../../VELUM/Velum/docs/SymbiontArchitecture_IsidaAdapter.md) и [`RefactoringPlan_SimplifyEnvironmentLayer.md`](../../../../VELUM/Velum/docs/RefactoringPlan_SimplifyEnvironmentLayer.md).*
