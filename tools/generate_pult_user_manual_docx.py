# -*- coding: utf-8 -*-
"""
Генерация пользовательской справки по пульту симбионта (AIStudio + движок isida).
...
def build_manual(doc: Document) -> None:
    ...
    tr = title.add_run(
        "Работа с пультом взаимодействия с симбионтом (AIStudio / ISIDA)"
    )
    tr.bold = True
    tr.font.size = Pt(16)
    ...
    add_para(
        doc,
        "Пульт — это часть вкладки «Симбионт»: вы задаёте словесные стимулы и отмечаете воздействия на «жизненные параметры» модели. "
    ...
    add_para(
        doc,
        "Пульт — это панель «Пульт взаимодействия с симбионтом» в разделе «Симбионт» главного окна AIStudio. "
        "Через него вы подаёте симбионту словесные сообщения, отмечаете типы внешних воздействий на его «жизненные параметры», "
        "задаёте тон и настроение реплики, при необходимости — цвет сцены для зрительного канала. "
        "Ниже по той же вкладке показываются параметры симбионта и его действия; пульт работает вместе с остальной средой моделирования.",
    )
    ...
    add_heading(doc, "3. Поле «Окно сообщения симбионту» и распознавание текста", 1)
    add_para(
        doc,
        "Вы вводите фразу так, как хотите её «сказать» симбионту. Справа от поля показывается «Распознанный текст»: "
        "слова, которые уже есть в словарном дереве симбионта, отображаются как есть; слова, которых в дереве ещё нет, "
    ...
    add_para(
        ...
        "как будто симбионт «видит» действие со стороны, без реального толчка по параметрам. При этом события восприятия и запись в память по фразе могут продолжаться — "
        "режим нужен, чтобы отработать сценарий «наблюдателя», не искажая показатели здоровья симбионта.",
    )
    ...
    add_para(
        doc,
        "В двух столбцах перечислены воздействия из файла проекта «InfluenceActions» — каждое связано с изменением тех или иных жизненных параметров симбионта. "
    ...
    add_para(
        doc,
        "Над блоком действий симбионта может отображаться полоска с таймером ожидания. В этом режиме система ждёт вашего решения или завершения процедуры оценки. "
    ...
    add_para(
        ...
        "Для этого нужно открыть вкладку «Симбионт», чтобы объект пульта был создан — иначе сценарий не сможет подать воздействия.",
    )
    add_para(
        doc,
    ...
    add_heading(doc, "12. Если симбионт «мёртв»", 1)
    add_para(
        doc,
        "При наступлении условий смерти симбионта пульт блокируется: воздействия не подаются, пока вы не восстановите состояние через настройки проекта или новый прогон по правилам вашей методики.",
    )
